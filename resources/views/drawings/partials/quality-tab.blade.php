<!-- resources/views/drawings/partials/quality-tab.blade.php -->
    ...
    <div class="flex items-center justify-center min-h-[400px]">
        <div class="text-center max-w-md mx-auto p-9">
            ...
            <p class="text-gray-600 mb-2">
                Anda tidak memiliki akses untuk melihat <span class="font-semibold text-gray-900">Quality</span>
            </p>
...
<div class="space-y-4">
    ...
    <div class="flex justify-between items-center mb-4">
        <h3 class="text-lg font-semibold text-gray-900">Quality Documents ({{ $files->count() }})</h3>
        @if (auth()->user()->role !== 'user')
            <button 
                class="upload-btn bg-yellow-600 hover:bg-yellow-700 text-white px-4 py-2 rounded-lg flex items-center transition shadow-sm"
                data-modal-id="uploadModalQuality">
                ...
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden group">
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex items-center justify-between text-xs text-gray-500 mb-3">
                            ...
                            <span class="px-2 py-0.5 bg-white-100 border border-yellow-200 rounded-full text-yellow-700 font-medium">{{ strtoupper($extension) }}</span>
                        </div>
                        @if($file->deskripsi)
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            ...
                            <button 
                                onclick="deleteFile('{{ route('drawings.deleteQuality', $file->id) }}', '{{ $file->nama }}')"
    ...
    @else
        ...
        <div class="text-center py-16 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border-2 border-dashed border-gray-300">
            ...
            <h3 class="text-xl font-semibold text-gray-900 mb-2">Belum ada Quality Document</h3>
            <p class="text-gray-500 mb-6 max-w-md mx-auto">Upload foto, video, atau dokumen quality document pertama Anda.</p>
            <button 
                class="upload-btn bg-yellow-600 hover:bg-yellow-700 text-white px-8 py-3 rounded-lg inline-flex items-center transition shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
                data-modal-id="uploadModalQuality">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                          d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
...
<!-- Upload Modal untuk Quality -->
<div id="uploadModalQuality" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onclick="if(event.target === this) closeModal('uploadModalQuality')">
    <div class="bg-white rounded-xl shadow-2xl max-w-md w-full p-6" onclick="event.stopPropagation()">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-xl font-bold text-gray-900">Upload Quality Document</h3>
            <button onclick="closeModal('uploadModalQuality')" class="text-gray-400 hover:text-gray-600 transition">
        ...
        <form id="uploadQualityForm" action="{{ route('drawings.uploadQuality', $drawing) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="space-y-4">
                <!-- File Input -->
                <div>
                    ...
                    <div class="relative">
                        <input 
                            ...
                            class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 transition file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-yellow-50 file:text-yellow-700 hover:file:bg-yellow-100">
                    </div>
                ...
                <div>
                    ...
                    <textarea 
                        ...
                        class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 transition resize-none"></textarea>
                </div>

                <!-- Progress Bar -->
                <div id="uploadQualityProgress" class="hidden">
                    <div class="mb-2 flex justify-between items-center">
                        <span class="text-sm font-medium text-gray-700">Uploading...</span>
                        <span id="uploadQualityPercent" class="text-sm font-bold text-yellow-600">0%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-3 overflow-hidden shadow-inner">
                        <div id="uploadQualityBar" 
                             class="h-3 rounded-full" 
                             style="width: 0%; background: linear-gradient(90deg, #D97706 0%, #B45309 100%) !important; transition: width 0.3s ease-out;"></div>
                    </div>
                    <p id="uploadQualityStatus" class="text-xs text-gray-500 mt-2">Preparing upload...</p>
                </div>
                ...
                <div class="flex gap-3 pt-4">
                    <button 
                        type="button" 
                        onclick="closeModal('uploadModalQuality')"
                        id="cancelQualityBtn"
                    ...
                    <button 
                        type="submit"
                        id="submitQualityBtn"
                        class="flex-1 bg-yellow-600 hover:bg-yellow-700 text-white px-4 py-3 rounded-lg transition font-medium shadow-md hover:shadow-lg">
                        Upload
                    </button>
                </div>
            </div>
