@if(!auth()->user()->hasPermission('setup_procedures', 'view'))
    <div class="flex items-center justify-center min-h-[400px]">
        <div class="text-center max-w-md mx-auto p-9">
            ...
            <p class="text-gray-600 mb-2">
                Anda tidak memiliki akses untuk melihat <span class="font-semibold text-gray-900">Setup Procedure</span>
            </p>
...
<div class="space-y-4">
    ...
    <div class="flex justify-between items-center mb-4">
        <h3 class="text-lg font-semibold text-gray-900">Setup Procedures ({{ $files->count() }})</h3>
        @if (auth()->user()->role !== 'user')
            <button 
                class="upload-btn bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg flex items-center transition shadow-sm"
                data-modal-id="uploadModalSetupProcedure">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                        d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                </svg>
                Upload Setup Procedure
            </button>
        @endif
    </div>
                ...
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden group">
                    ...
                    <div class="p-4">
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            ...
                            <button 
                                onclick="deleteFile('{{ route('drawings.deleteSetupProcedure', $file->id) }}', '{{ $file->nama }}')"
    ...
    @else
        ...
        <div class="text-center py-16 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border-2 border-dashed border-gray-300">
            ...
            <h3 class="text-xl font-semibold text-gray-900 mb-2">Belum ada Setup Procedure</h3>
            <p class="text-gray-500 mb-6 max-w-md mx-auto">Upload setup procedure pertama Anda untuk dokumentasi proses setup.</p>
            <button 
                class="upload-btn bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-lg inline-flex items-center transition shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
                data-modal-id="uploadModalSetupProcedure">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                          d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
...
<!-- Upload Modal untuk Setup Procedure -->
<div id="uploadModalSetupProcedure" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onclick="if(event.target === this) closeModal('uploadModalSetupProcedure')">
    <div class="bg-white rounded-xl shadow-2xl max-w-md w-full p-6" onclick="event.stopPropagation()">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-xl font-bold text-gray-900">Upload Setup Procedure</h3>
            <button onclick="closeModal('uploadModalSetupProcedure')" class="text-gray-400 hover:text-gray-600 transition">
        ...
        <form id="uploadSetupProcedureForm" action="{{ route('drawings.uploadSetupProcedure', $drawing) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="space-y-4">
                <!-- File Input -->
                <div>
                    ...
                    <div class="relative">
                        <input 
                            type="file" 
                            name="file" 
                            id="fileSetupProcedureInput"
                            required
                            accept="image/*,video/*,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx"
                            class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
                    </div>
                ...
                <div>
                    ...
                    <textarea 
                        name="deskripsi" 
                        rows="3"
                        placeholder="Deskripsi setup procedure (opsional)"
                        class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition resize-none"></textarea>
                </div>

                <!-- Progress Bar -->
                <div id="uploadSetupProcedureProgress" class="hidden">
                    <div class="mb-2 flex justify-between items-center">
                        <span class="text-sm font-medium text-gray-700">Uploading...</span>
                        <span id="uploadSetupProcedurePercent" class="text-sm font-bold text-gray-600">0%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-3 overflow-hidden shadow-inner">
                        <div id="uploadSetupProcedureBar" 
                             class="h-3 rounded-full transition-all duration-300 ease-out"
                             style="width: 0%; background: linear-gradient(90deg, #6366F1 0%, #4F46E5 100%);"></div>
                    </div>
                    <p id="uploadSetupProcedureStatus" class="text-xs text-gray-500 mt-2">Preparing upload...</p>
                </div>
                ...
                <div class="flex gap-3 pt-4">
                    <button 
                        type="button" 
                        onclick="closeModal('uploadModalSetupProcedure')"
                        id="cancelSetupProcedureBtn"
                        class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-3 rounded-lg transition font-medium">
                        Batal
                    </button>
                    <button 
                        type="submit"
                        id="submitSetupProcedureBtn"
                        class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-3 rounded-lg transition font-medium shadow-md hover:shadow-lg">
                        Upload
                    </button>
                </div>
            </div>
...
<style>
    ...
    #uploadSetupProcedureBar { animation: pulse-progress 2s ease-in-out infinite; }
</style>
...
<script>
    ...
    function closeModal(id) {
        ...
        if (modal) {
            ...
            if (id === 'uploadModalSetupProcedure') {
                const form = document.getElementById('uploadSetupProcedureForm');
                if (form) form.reset();
                const progress = document.getElementById('uploadSetupProcedureProgress');
                if (progress) progress.classList.add('hidden');
                document.getElementById('uploadSetupProcedureBar').style.width = '0%';
                document.getElementById('uploadSetupProcedurePercent').textContent = '0%';
                document.getElementById('uploadSetupProcedureStatus').textContent = 'Preparing upload...';
            }
        }
    }

    // Upload dengan Progress Bar
    document.getElementById('uploadSetupProcedureForm')?.addEventListener('submit', function(e) {
        e.preventDefault();

        const form = this;
        const formData = new FormData(form);
        const submitBtn = document.getElementById('submitSetupProcedureBtn');
        const cancelBtn = document.getElementById('cancelSetupProcedureBtn');
        const progressContainer = document.getElementById('uploadSetupProcedureProgress');
        const progressBar = document.getElementById('uploadSetupProcedureBar');
        const progressPercent = document.getElementById('uploadSetupProcedurePercent');
        const progressStatus = document.getElementById('uploadSetupProcedureStatus');
        ...
        xhr.addEventListener('load', () => {
            if (xhr.status === 200) {
                ...
                if (data.success) {
                    ...
                    setTimeout(() => {
                        closeModal('uploadModalSetupProcedure');
                        showNotification(data.message || 'Setup Procedure berhasil diupload!', 'success');
        ...
        function resetUpload() {
            ...
            progressBar.style.background = 'linear-gradient(90deg, #6366F1 0%, #4F46E5 100%)';
        }
    });

    // Auto-fill nama dari filename
    document.getElementById('fileSetupProcedureInput')?.addEventListener('change', function(e) {
