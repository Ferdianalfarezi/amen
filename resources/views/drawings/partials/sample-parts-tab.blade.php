@if(!auth()->user()->hasPermission('sample_parts', 'view'))
    <div class="flex items-center justify-center min-h-[400px]">
        <div class="text-center max-w-md mx-auto p-9">
            ...
            <p class="text-gray-600 mb-2">
                Anda tidak memiliki akses untuk melihat <span class="font-semibold text-gray-900">Sample Parts</span>
            </p>
...
<div class="space-y-4">
    ...
    <div class="flex justify-between items-center mb-4">
        <h3 class="text-lg font-semibold text-gray-900">Sample Parts ({{ $files->count() }})</h3>
        @if (auth()->user()->role !== 'user')
            <button 
                class="upload-btn bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg flex items-center transition shadow-sm"
                data-modal-id="uploadModalSamplePart">
                ...
                @php
                    ...
                    switch ($extension) {
                        ...
                        case 'pptx':
                            ...
                            $iconColor = 'text-orange-600';  // ← ganti dari 500 ke 600
                            $bgColor = 'bg-orange-100';      // ← ganti dari 50 ke 100
                    ...
                    <div class="p-4">
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            <a href="{{ asset('storage/' . $file->file_path) }}" 
                               download="{{ $file->nama }}"
                               class="col-span-2 bg-green-500 hover:bg-green-600 text-white px-3 py-2.5 rounded-lg flex items-center justify-center transition-all shadow-sm hover:shadow-md text-sm font-medium"
                            ...
                            <button 
                                onclick="deleteFile('{{ route('drawings.deleteSamplePart', $file->id) }}', '{{ $file->nama }}')"
        ...
        <div class="text-center py-16 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border-2 border-dashed border-gray-300">
            <div class="inline-block p-6 bg-white rounded-full shadow-md mb-4">
                <svg class="w-16 h-16 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2m-2 8h.01" />
                </svg>
            </div>
            <h3 class="text-xl font-semibold text-gray-900 mb-2">Belum ada Sample Part</h3>
            <p class="text-gray-500 mb-6 max-w-md mx-auto">Upload foto, video, atau dokumen sample part pertama Anda.</p>
            <button 
                class="upload-btn bg-green-600 hover:bg-green-700 text-white px-8 py-3 rounded-lg inline-flex items-center transition shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
                data-modal-id="uploadModalSamplePart">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                          d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
...
<!-- Upload Modal untuk Sample Part -->
<div id="uploadModalSamplePart" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onclick="if(event.target === this) closeModal('uploadModalSamplePart')">
    <div class="bg-white rounded-xl shadow-2xl max-w-md w-full p-6" onclick="event.stopPropagation()">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-xl font-bold text-gray-900">Upload Sample Part</h3>
            <button onclick="closeModal('uploadModalSamplePart')" class="text-gray-400 hover:text-gray-600 transition">
        ...
        <form id="uploadSamplePartForm" action="{{ route('drawings.uploadSamplePart', $drawing) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="space-y-4">
                <!-- File Input -->
                <div>
                    ...
                    <div class="relative">
                        <input 
                            type="file" 
                            name="file" 
                            id="fileSamplePartInput"
                            required
                            accept="image/*,video/*,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx"
                            class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-green-50 file:text-green-700 hover:file:bg-green-100">
                    </div>
                ...
                <div>
                    ...
                    <textarea 
                        ...
                        class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition resize-none"></textarea>
                </div>
                ...
                <div class="flex gap-3 pt-4">
                    <button 
                        type="button" 
                        onclick="closeModal('uploadModalSamplePart')"
                        class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-3 rounded-lg transition font-medium">
                        Batal
                    </button>
                    <button 
                        type="submit"
                        class="flex-1 bg-green-600 hover:bg-green-700 text-white px-4 py-3 rounded-lg transition font-medium shadow-md hover:shadow-lg">
                        Upload
                    </button>
                </div>
            </div>
...
<script>
    ...
    // Upload form handler
    document.getElementById('uploadSamplePartForm')?.addEventListener('submit', function(e) {
        e.preventDefault();
        ...
        .then(data => {
            if (data.success) {
                closeModal('uploadModalSamplePart');
                this.reset();
                
                const notif = document.createElement('div');
                notif.className = 'fixed top-4 right-4 bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg z-[200] flex items-center gap-2';
    ...
    document.getElementById('fileSamplePartInput')?.addEventListener('change', function(e) {
