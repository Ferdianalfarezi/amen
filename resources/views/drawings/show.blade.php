...
<script>
    ...
    // ===================================================================
    // UNIVERSAL UPLOAD HANDLER WITH PROGRESS BAR (ALL TABS)
    // ===================================================================
    document.addEventListener('submit', function(e) {
        const formId = e.target.id;
        
        // Check if it's an upload form (matches pattern: uploadXXXForm)
        if (formId && formId.match(/^upload\w+Form$/)) {
            e.preventDefault();
            
            // Extract tab type from form ID (e.g., upload2DForm -> 2D, upload3DForm -> 3D)
            const tabType = formId.replace('upload', '').replace('Form', '');
            
            const form = e.target;
            const formData = new FormData(form);
            const submitBtn = document.getElementById(`submit${tabType}Btn`);
            const cancelBtn = document.getElementById(`cancel${tabType}Btn`);
            const progressContainer = document.getElementById(`upload${tabType}Progress`);
            const progressBar = document.getElementById(`upload${tabType}Bar`);
            const progressPercent = document.getElementById(`upload${tabType}Percent`);
            const progressStatus = document.getElementById(`upload${tabType}Status`);
            const fileInput = document.getElementById(`file${tabType}Input`);
            const modalId = `uploadModal${tabType}`;
            ...
            // XMLHttpRequest untuk tracking progress
            const xhr = new XMLHttpRequest();
            
            // Track upload progress
            xhr.upload.addEventListener('progress', function(e) {
                if (e.lengthComputable && progressBar && progressPercent && progressStatus) {
                    const percentComplete = Math.round((e.loaded / e.total) * 100);
                    
                    // Update width dan pastikan background tetap
                    ...
                    progressStatus.textContent = `${uploadedMB} MB of ${totalMB} MB uploaded`;
            ...
            xhr.addEventListener('error', () => handleUploadError('Network error occurred'));
            xhr.addEventListener('abort', () => handleUploadError('Upload cancelled'));
    ...
    // ===================================================================
    // UNIVERSAL FILE INPUT CHANGE HANDLER (Auto-fill nama for all tabs)
    // ===================================================================
    document.addEventListener('change', function(e) {
        // Check if it's a file input (matches pattern: fileXXXInput)
        if (e.target.id && e.target.id.match(/^file\w+Input$/) && e.target.files.length > 0) {
            const fileName = e.target.files[0].name;
            const nameInput = e.target.form?.querySelector('input[name="nama"]');
    ...
    document.addEventListener('keydown', (event) => {
        if (event.key === 'Escape') {
            ...
            const allProgressContainers = document.querySelectorAll('[id$="Progress"]');
            ...
            // Don't close upload modals if upload is in progress
            if (!uploadInProgress) {
